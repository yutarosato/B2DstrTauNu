...
Int_t iterative_fit( TH1D* hist, TF1* func, Int_t nparam, Double_t* init, const Char_t* opt_plot, Int_t max ){
  return iterative_fit( hist, func, nparam, init, opt_plot, NULL, max );
}

Int_t iterative_fit( TH1D* hist, TF1* func, Int_t nparam, Double_t* init, const Char_t* opt_plot, fit_iter manip, Int_t max ){
  if( manip!=NULL ) manip(func);
  //TFitResultPtr fit_result = hist->Fit(func, "RQSU", opt_plot );
  //TFitResultPtr fit_result = hist->Fit(func, "LRSU", opt_plot );
  TFitResultPtr fit_result = hist->Fit(func, "QLRS", opt_plot );
  ...
  if( fit_status==0 ){
    fit_result = hist->Fit(func, "LRS", opt_plot );
    fit_result->Print();
    //fit_result->Print("V"); // including covariance matrix
    std::cout << "fit_status = " << fit_status << "(" << (Int_t)fit_result->IsValid() << ")" << std::endl // IsValid : 1(successful fit), 0(falsely fit)
  ...
  while( fit_status!=0 || cnt_v!=0 ){
    ...
    //if( cnt_v<0 ) fit_result = hist->Fit( func,"LRSU",  opt_plot );
    //else          fit_result = hist->Fit( func,"LRQSU", opt_plot );
    
    if( cnt_v<0 ){
      fit_result = hist->Fit( func,"LRQSU",  opt_plot );
      if( fit_result==0 ) fit_result = hist->Fit( func,"LRSU",  opt_plot );
    }else fit_result = hist->Fit( func,"LRQSU", opt_plot );
...
fit_func make_func( Int_t select ){
  ...
  case 30001:
    ...
    break;
  case 33001:
    func = func_cb_bif2_straight;
    break;
  case 30002:
    func = func_cb_bif_parabola;
    break;
  case 33002:
    func = func_cb_bif2_parabola;
    break;
  case 30008:
    func = func_cb_bif_parabola_pdf;
    break;
  case 40000:
    func = func_cb_bif_gauss;
    break;
  case 40002:
    func = func_cb_bif_gauss_parabola;
    break;
  case 40081:
    func = func_cb_bif_gauss_threshold;
    break;
  default:
...
void func_set_parameters( Int_t select, TF1* func,   TH1D* hist,
  ...
  case 20: // double gaussian
    ...
    func->SetParameters(area,     0.6,        mu,     sigma,     0.4 );
    break;
  ...
  case 30000: // cb( bifuracated gaussian )
    ...
    func->SetParLimits(3, -0.01, 0.01);
    break;
  case 33000: // cb( bifuracated gaussian )
    sigma = 4.0*hist->GetBinWidth(1);
    mu    = hist->GetBinCenter(hist->GetMaximumBin());
    area  = hist->GetMaximum()*sqrt(TMath::TwoPi())*sigma;
    func->SetParNames  ("area","alpha_l","n_l","mean","sigma_l","alpha_h","n_h","sigma_ratio");
    func->SetParameters(area,      1.5,   1,  mu,  sigma,        2.5,     0.5,    1.0);
    func->SetParLimits(3, -0.01, 0.01);
    break;
  case 30001: // cb( bifuracated gaussian ) + straight
    ...
    func->SetParLimits(3, -0.01, 0.01);
    break;
  case 33001: // cb( bifuracated gaussian ) + straight
    sigma = 4.0*hist->GetBinWidth(1);
    mu    = hist->GetBinCenter(hist->GetMaximumBin());
    area  = hist->GetMaximum()*sqrt(TMath::TwoPi())*sigma;
    a = ( hist->GetBinContent(xbin)-hist->GetBinContent(1))/(hist->GetBinCenter(xbin)-hist->GetBinCenter(1) );
    b = hist->GetBinContent(1)-a*hist->GetBinCenter(1);
    func->SetParNames  ("area","alpha_l","n_l","mean","sigma_l","alpha_h","n_h","sigma_ratio", "slope", "offset");
    func->SetParameters(area,      1.5,   1,  mu,  sigma,        2.5,     0.5,    1.0,     a,         b);
    func->SetParLimits(3, -0.01, 0.01);
    break;
  case 30002: // cb( bifuracated gaussian ) + parabola
    sigma = 4.0*hist->GetBinWidth(1);
    mu    = hist->GetBinCenter(hist->GetMaximumBin());
    area  = hist->GetMaximum()*sqrt(TMath::TwoPi())*sigma;
    a = 0;
    b = ( hist->GetBinContent(xbin)-hist->GetBinContent(1))/(hist->GetBinCenter(xbin)-hist->GetBinCenter(1) );
    c= hist->GetBinContent(1)-a*hist->GetBinCenter(1);
    func->SetParNames  ("area","alpha_l","n_l","mean","sigma_l","alpha_h","n_h","sigma_h", "a", "b", "c" );
    func->SetParameters(area,      1.5,   1,  mu,  sigma,        2.5,     0.5,    sigma,     a,  b,   c  );
    func->SetParLimits(3, -0.01, 0.01);
    break;
  case 33002: // cb( bifuracated gaussian ) + parabola
    sigma = 4.0*hist->GetBinWidth(1);
    mu    = hist->GetBinCenter(hist->GetMaximumBin());
    area  = hist->GetMaximum()*sqrt(TMath::TwoPi())*sigma;
    a = 0;
    b = ( hist->GetBinContent(xbin)-hist->GetBinContent(1))/(hist->GetBinCenter(xbin)-hist->GetBinCenter(1) );
    c= hist->GetBinContent(1)-a*hist->GetBinCenter(1);
    func->SetParNames  ("area","alpha_l","n_l","mean","sigma_l","alpha_h","n_h","sigma_ratio", "a", "b", "c" );
    func->SetParameters(area,      1.5,   1,  mu,  sigma,        2.5,     0.5,    1.0,     a,  b,   c  );
    func->SetParLimits(3, -0.01, 0.01);
    break;
  default:
    std::cerr << "indefinite fit function -> abort()" << std::endl;
...
// 7. straight line for PDF
// p[0]=area, p[1]=offset,
Double_t func_straight_pdf(const Double_t* x, const Double_t* p){
  Double_t f = p[0]*(x[0]+p[1]);
  return f;
}

// 8. parabola for PDF
// p[0]=area, p[1]=slope, p[2]=offset
Double_t func_parabola_pdf(const Double_t* x, const Double_t* p){
  Double_t f = p[0]*(x[0]*x[0]+p[1]*x[0]+p[2]);
  return f;
}
...
// 200. bif-gaussian + gaussian (different parameterization with 110)
// p[0]=area, p[1]=area_ratio, p[2]=mean, p[3]=sigma, p[4]=sigma_ratio(low), p[5]=sigma_ratio(high)
Double_t func_bf2gauss(const Double_t* x, const Double_t* p){
  Double_t f = p[0]*  p[1]  /sqrt(TMath::TwoPi())/(p[3]     ) * exp( -0.5 * ((x[0]-p[2])/(p[3]     )) * ((x[0]-p[2])/(p[3]     )) );
  if( x[0]<p[1] ) f += 2*p[0]*(1-p[1])/sqrt(TMath::TwoPi())/(p[3]*p[4]+p[3]*p[5]) * exp( -0.5 * ((x[0]-p[2])/(p[3]*p[4])) * ((x[0]-p[2])/(p[3]*p[4])) );
  else            f += 2*p[0]*(1-p[1])/sqrt(TMath::TwoPi())/(p[3]*p[4]+p[3]*p[5]) * exp( -0.5 * ((x[0]-p[2])/(p[3]*p[5])) * ((x[0]-p[2])/(p[3]*p[5])) );
  return f;
}
...
// 33000. cb function (bifurcated gaussian)  // different parameterization with 30000
// p[0]=area(gaussian)
// p[1]=alpha_low(range of gauss), p[2]=n_low(tail shape),
// p[3]=xbar(mean), p[4]=sigma_low
// p[5]=alpha_high(range of gauss), p[6]=n_high(tail shape),
// p[7]=sigma_ratio(high/low)
Double_t func_cb_bif2(const Double_t* x, const Double_t*p){
  Double_t z   = x[0]-p[3];
  Double_t dim_l = 1.0+p[1]/p[2]*fabs(z)/(p[4]     )-p[1]*p[1]/p[2];
  Double_t dim_h = 1.0+p[5]/p[6]*fabs(z)/(p[4]*p[7])-p[5]*p[5]/p[6];
  Double_t f;
  if(      z <-p[4]     *p[1] ) f = p[0]/sqrt(TMath::TwoPi())/p[4]*exp(-0.5*p[1]*p[1])/pow(dim_l,p[2]); // low-side tail
  else if( z > p[4]*p[7]*p[5] ) f = p[0]/sqrt(TMath::TwoPi())/p[4]*exp(-0.5*p[5]*p[5])/pow(dim_h,p[6]); // high-side tail
  else if( z < 0              ) f = p[0]/sqrt(TMath::TwoPi())/p[4]*exp(-0.5*z*z/(p[4]*p[4]));           // gaussian (low-side)
  else                          f = p[0]/sqrt(TMath::TwoPi())/p[4]*exp(-0.5*z*z/(p[7]*p[7]*p[4]*p[4])); // gaussian (high-side)
  return f;
}
...
// 33001. cb functione (bifurcated gaussian)(p[0-7]) + straight(p[8-9])
Double_t func_cb_bif2_straight(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif2(x,&p[0]) + func_straight(x,&p[8]);
  return f;
}

// 30002. cb functione (bifurcated gaussian)(p[0-7]) + parabola(p[8-10])
Double_t func_cb_bif_parabola(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif(x,&p[0]) + func_parabola(x,&p[8]);
  return f;
}

// 33002. cb functione (bifurcated gaussian)(p[0-7]) + parabola(p[8-10])
Double_t func_cb_bif2_parabola(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif2(x,&p[0]) + func_parabola(x,&p[8]);
  return f;
}

// 30008. cb functione (bifurcated gaussian)(p[0-7]) + parabola_pdf(p[8-10])
Double_t func_cb_bif_parabola_pdf(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif(x,&p[0]) + func_parabola_pdf(x,&p[8]);
  return f;
}

// 40000. cb function (bifurecated both-side tail) + gauss
// p[0]=area(gaussian)
// p[1]=area_ratio
// p[2]=sigma
// p[3]=xbar(mean),
// p[4]=alpha_low (range of gauss), p[5]=n_low (tail shape), p[6]=ratio_sigma_low
// p[7]=alpha_high(range of gauss), p[8]=n_high(tail shape), p[9]=ratio_sigma_high

Double_t func_cb_bif_gauss(const Double_t* x, const Double_t*p){
  Double_t z   = x[0]-p[3];
  Double_t dim_l = 1.0+p[4]/p[5]*fabs(z)/(p[2]*p[6])-p[4]*p[4]/p[5];
  Double_t dim_h = 1.0+p[7]/p[8]*fabs(z)/(p[2]*p[9])-p[7]*p[7]/p[8];
  Double_t f;
  if(      z <-p[2]*p[6]*p[4] ) f = p[0]*(1-p[1])/sqrt(TMath::TwoPi())/p[2]/p[6]*exp(-0.5*p[4]*p[4])/pow(dim_l,p[5]); // low-side tail
  else if( z > p[2]*p[9]*p[7] ) f = p[0]*(1-p[1])/sqrt(TMath::TwoPi())/p[2]/p[6]*exp(-0.5*p[7]*p[7])/pow(dim_h,p[8]); // high-side tail
  else if( z < 0              ) f = p[0]*(1-p[1])/sqrt(TMath::TwoPi())/p[2]/p[6]*exp(-0.5*z*z/(p[2]*p[2]*p[6]*p[6])); // gaussian (low-side)
  else                          f = p[0]*(1-p[1])/sqrt(TMath::TwoPi())/p[2]/p[6]*exp(-0.5*z*z/(p[2]*p[2]*p[9]*p[9])); // gaussian (high-side)


  f += p[0]*p[1]/sqrt(TMath::TwoPi())/p[2]*exp(-0.5*((x[0]-p[3])/p[2])*((x[0]-p[3])/p[2]));  
  return f;
}

// 40002. cb functione (bifurcated gaussian) + gaussian(p[0-9]) + parabola(p[10-12])
Double_t func_cb_bif_gauss_parabola(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif_gauss(x,&p[0]) + func_parabola(x,&p[10]);
  return f;
}

// 40081. cb functione (bifurcated gaussian) + gaussian(p[0-9]) + threshold(p[10-12])
Double_t func_cb_bif_gauss_threshold(const Double_t* x, const Double_t*p){
  Double_t f = func_cb_bif_gauss(x,&p[0]) + func_threshold1(x,&p[10]);
  return f;
}


// 81. threshold function // modified for D* analysis from Xsll analysis @20141218
// p[0]=norm, p[1]=threshold, p[2]=k(shape)
Double_t func_threshold1( const Double_t* x, const Double_t* p ){
  Double_t f;
  /*
    if( x[0]<p[1] ){
    //f = p[0]*x[0]*sqrt(1-x[0]*x[0]/(p[1]*p[1]))*exp(p[2]*(1-x[0]/p[1])*(1-x[0]/p[1]));
    //f = p[0]*sqrt(1-x[0]*x[0]/(p[1]*p[1]))*exp((p[1]-x[0])/p[2]); // threshold function in delta-M fitting for (Mbc)
  }else{
  f = 0;
  }
  */
  if( x[0]>p[1] ){
    //f = p[0]*sqrt(x[0]*x[0]/(p[1]*p[1])-1)*exp((x[0]-p[1])/p[2]); // threshold function in delta-M fitting
    //f = p[0]*( pow(x[0]-p[1], 0.5) + p[2]*pow(x[0]-p[1], 1.5) ); // threshold function in delta-M fitting
    //f = p[0]*( pow(x[0]-p[1], 0.5) + p[2]*pow(x[0]-p[1], 1.5) + p[3]*pow(x[0]-p[1], 2.5) ); // threshold function in delta-M fitting
    f = p[0]*( pow(x[0]-p[1], 0.5) + p[2]*pow(x[0]-p[1], 1.5) + p[3]*pow(x[0]-p[1], 0.25) ); // threshold function in delta-M fitting
  }else{
    f = 0;
  }
  return f;
