...
const Int_t   smpoint  = 4;
const Char_t* model    = "S_{1}-LQ";
const Char_t* param    = "C_{T}";
const Char_t* unit     = "";
const Char_t* filename = "S1LQ";
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  TGraph* g_78m_band2 = makeBand( g_78m, 7 );
  g_78m_band2->SetFillColor(1);
  g_78m_band2->SetFillStyle(3005);
  
  for( Int_t ip=0; ip<g_78m_band->GetN()/2; ip++ ) g_78m->SetPointError( ip, 0.0, 0.0 );


  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  const Int_t   npoint           = 12; // last component is dummy data for smooth curve
  Double_t      x       [npoint] = { -0.150, -0.100, -0.040, -0.030, 0.000, 0.090, 0.180, 0.260, 0.340, 0.360, 0.400, 1.00 };
  Double_t      nsig[ng][npoint] = { // S1-LQ
    {183.869, 183.943, 185.41,  185.71,  186.799, 196.627, 204.515, 207.247, 206.698, 206.544, 205.831, 205.831 },
    {115.811, 115.353, 115.644, 115.798, 116.126, 122.014, 127.803, 130.335, 130.719, 130.747, 130.505, 130.505 },
    { 68.0579, 68.5901, 69.7656, 69.9115, 70.6731, 74.6133, 76.7122, 76.9121, 75.9978, 75.7973, 75.326,  75.326 },
  };

  Double_t      nsigE[ng][npoint] = { // S1-LQ
    {0.771577, 0.655495, 0.460733, 0.423756, 0.0, 0.722069, 0.969887, 1.11786,  1.19746,  1.21201,  1.23133, 1.23133  },
    {0.625963, 0.530137, 0.371736, 0.34118,  0.0, 0.570055, 0.776432, 0.897926, 0.964825, 0.976568, 0.992831,0.992831 },
    {0.451111, 0.385523, 0.272191, 0.251327, 0.0, 0.443193, 0.581234, 0.665843, 0.709248, 0.717826, 0.728321,0.728321 },
  };
  ...
  for( Int_t ig=0; ig<ng; ig++ ){
    ...
    g_78m_band     ->Draw("FLsame");
    g_78m_band2    ->Draw("FLsame");
    g_78m          ->Draw(  "same");
    smline         ->Draw( "Lsame");
  ...
  for( Int_t ig=0; ig<ng; ig++ ){
    ...
    for( Int_t ip=0; ip<g_78m_band->GetN()/2; ip++ ){
      Double_t x = g_78m_band->GetX()[ip];
      Double_t y_th   = g_78m->Eval(x);
      Double_t yeh_th = g_78m_band->GetY()[ip] - y_th;
      Double_t yel_th = y_th - g_78m_band->GetY()[g_78m_band->GetN()-ip-1];
      if( x - g_78m_band->GetX()[ip] > 1.0e-5 ||
	  x - g_78m_band->GetX()[g_78m_band->GetN()-ip-1] >1.0e-5) std::cerr << "[ABORT] Wrong x values : "
										 << ip << " : "
										 << x  << " : "
										 << g_78m_band->GetX()[ip] << " : "
										 << g_78m_band->GetX()[g_78m_band->GetN()-ip-1] << " : "
										 << std::endl, abort();
