...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  pi0_eff*** pi0_syst = new pi0_eff**[nlep]; // [e,m,l][modes]
  for( Int_t ilep=0; ilep<nlep; ilep++ ){
    pi0_syst[ilep] = new pi0_eff*[nd_dec];
    for( Int_t idec=0; idec<nd_dec; idec++ ){
      pi0_syst[ilep][idec] = new pi0_eff();
    }
  }
  
  // +++++++++++++++++++++++ add pi0 ++++++++++++++++++++++++++++++++
  Double_t cnt    [nlep][nd_dec]={0}; // [e,m,l][modes]
  Double_t cnt_tot[nlep]        ={0}; // [e,m,l][modes]
  ...
  for( Int_t m=0; m<nentries; m++ ){
    ...
    for( int ich=0; ich<4; ich++ ){
      if( tree->GetLeaf(Form("d1ch%drec",ich))->GetValue() != 111 ) continue;
      pi0_syst[fl_lep][fl_dec]->add( tree->GetLeaf( Form("d1ch%dp", ich))->GetValue() );
      pi0_syst[     2][fl_dec]->add( tree->GetLeaf( Form("d1ch%dp", ich))->GetValue() );
    }
    cnt    [fl_lep][fl_dec]++;
    cnt    [     2][fl_dec]++;
    cnt_tot[fl_lep]++;
  ...
  for( Int_t ilep=0; ilep<nlep; ilep++ ){
    for( Int_t idec=0; idec<nd_dec; idec++ ){
      ...
      if( cnt[ilep][idec] && pi0_syst[ilep][idec]->get_totalNum() ){
	ratio [ilep][idec] = pi0_syst[ilep][idec]->total_effFactor();
	ratioE[ilep][idec] = pi0_syst[ilep][idec]->total_errFactor();
      }
  ...
  // ++++++++++++++++ make plots +++++++++++++++++++++++++
  TH1D** hist = makeHist( "pi0", 100, 0, 100, nlep); // [lep]
  for( Int_t ilep=0; ilep<nlep; ilep++ ){
    for( Int_t idec=0; idec<nd_dec; idec++ ){
  ...
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++
  TFile outfile( "pic/syst_pi0.root", "RECREATE" );
  can->Write();
  for( Int_t ilep=0; ilep<nlep; ilep++ ) hist[ilep]->Write();

  outfile.Close();
  can->Print( "pic/syst_pi0.eps" ); 
