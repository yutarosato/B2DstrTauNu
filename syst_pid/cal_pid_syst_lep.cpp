...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  //const TCut cut = "-10<cosdl1 && cosdl1<5 && self==2 && l1pid>0.90"; // tight lepton PID selection
  //const TCut cut = "-10<cosdl1 && cosdl1<5 && self==2";
  TTree* tree = new TTree();
  tree = c->CopyTree( Form("-10<cosdl1 && cosdl1<5 && self==2 && l1pid>%f",lepid) );
  ...
  for( Int_t m=0; m<nentries; m++ ){
    ...
    lid_syst[fl_exp][fl_lep]->addtrack( tree->GetLeaf( "l1p" )->GetValue(), tree->GetLeaf( "l1pc" )->GetValue() );
    cnt     [fl_exp][fl_lep]++;
    cnt_tot         [fl_lep]++;
  }
  ...
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++
  TFile outfile( Form("pic/syst_lep_%.2f.root",lepid), "RECREATE" );
  can->Write();
  hist->Write();
  outfile.Close();
  can->Print( Form("pic/syst_lep_%.2f.eps",lepid) ); 
