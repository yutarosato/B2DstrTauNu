...
void Ks_eff::add(double p, double cos)
{
  ...
  if ( p <= 0.5                ) p_index =  0;
  if ( (0.5 < p) && (p <= 1.5) ) p_index =  1;
  if ( 1.5 < p                 ) p_index = 2;

  // check for cos(theta) index
  if ( cos <= -0.5 )                  cos_index = 0;
  if ( (-0.5 < cos) &&  (cos <= 0.) ) cos_index = 1;
  if ( (0. < cos ) &&  (cos <= 0.5) ) cos_index = 2;
  if ( 0.5 < cos )                    cos_index = 3;
