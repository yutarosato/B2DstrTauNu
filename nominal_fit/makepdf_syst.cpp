...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(app.Argc()==5 || app.Argc()==6) )
    std::cerr << "Wrong input" << std::endl
	      << "Usage : " << app.Argv(0)
	      << " (int)ctgry (char)param (int)fl_multi (int)tag_cnt [(int)fl_lep]" << std::endl
  ...
  Int_t fl_multi = atoi(app.Argv(3));
  Int_t tag_cnt  = atoi(app.Argv(4));
  Int_t fl_lep = 0;
  if( app.Argc()==6 ) fl_lep = atoi(app.Argv(5));

  unsigned int seed = time(NULL);
  seed += 1000*tag_cnt;
  gRandom->SetSeed( seed );
  if( fl_multi && fl_ctgry>4 ){
    Read_Data( "calib.dat" );
    Fluct_calib(true);
  ...
  for( Int_t ievt=0; ievt<c_gmc_cut->GetEntries(); ievt++ ){
    Double_t weight = cal_weight( c_gmc_cut, ievt, (Double_t)nstream_gmc ); // nominal
    if    ( fl_multi    && (fl_ctgry==2 || fl_ctgry==3) ) weight  = cal_weight      ( c_gmc_cut, ievt, (Double_t)nstream_gmc, true ); // syst from semileptonic decay modelling
    else if(fl_multi==0 && (fl_ctgry==3 || fl_ctgry==4) ) weight *= cal_weight_syst1( c_gmc_cut, ievt, tag_cnt ); // syst from D** composition or Double D
    else if(fl_multi==0 &&  fl_ctgry==2                 ) weight *= cal_weight_syst2( c_gmc_cut, ievt          ); // syst from cos resolution (norm)
    if      ( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==7 && c_gmc_cut->GetLeaf("ctgrsyst")->GetValue()==7 ){
      ;
    }else if( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==8 && c_gmc_cut->GetLeaf("ctgrsyst")->GetValue()==8 ){
      ;
    }else if( fl_2sveto &&
    ...
    /*
    if( !(fl_multi==0 && fl_ctgry==3 && tag_cnt!=0)
	&& fl_2sveto &&
	( c_gmc_cut->GetLeaf("ctgrsyst")->GetValue()==7 ||
	  c_gmc_cut->GetLeaf("ctgrsyst")->GetValue()==8 )
	) weight = 0;
  ...
  for( Int_t ievt=0; ievt<c_dststmc_cut->GetEntries(); ievt++ ){
    Double_t weight = cal_weight( c_dststmc_cut, ievt, (Double_t)nstream_dststmc ); // nominal
    if    ( fl_multi    && (fl_ctgry==2 || fl_ctgry==3) ) weight  = cal_weight      ( c_dststmc_cut, ievt, (Double_t)nstream_gmc, true ); // syst from semileptonic decay modelling
    else if(fl_multi==0 && (fl_ctgry==3 || fl_ctgry==4) ) weight *= cal_weight_syst1( c_dststmc_cut, ievt, tag_cnt ); // syst from D** composition or Double D
    else if(fl_multi==0 &&  fl_ctgry==2                 ) weight *= cal_weight_syst2( c_dststmc_cut, ievt          ); // syst from cos resolution (norm)
    if      ( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==7 && c_dststmc_cut->GetLeaf("ctgrsyst")->GetValue()==7 ){
      ;
    }else if( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==8 && c_dststmc_cut->GetLeaf("ctgrsyst")->GetValue()==8 ){
      ;
    }else if( fl_2sveto &&
  ...
  for( Int_t ievt=0; ievt<c_npmc_cut->GetEntries(); ievt++ ){
    Double_t weight = cal_weight( c_npmc_cut, ievt, (Double_t)nstream_npmc ); // nominal
    if    ( fl_multi    && (fl_ctgry==2 || fl_ctgry==3) ) weight  = cal_weight      ( c_npmc_cut, ievt, (Double_t)nstream_gmc, true ); // syst from semileptonic decay modelling
    else if(fl_multi==0 && (fl_ctgry==3 || fl_ctgry==4) ) weight *= cal_weight_syst1( c_npmc_cut, ievt, tag_cnt ); // syst from D** composition or Double D
    else if(fl_multi==0 &&  fl_ctgry==2                 ) weight *= cal_weight_syst2( c_npmc_cut, ievt          ); // syst from cos resolution (norm)
    if      ( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==7 && c_npmc_cut->GetLeaf("ctgrsyst")->GetValue()==7 ){
      ;
    }else if( fl_multi==0 && fl_ctgry==3 && abs(tag_cnt)==8 && c_npmc_cut->GetLeaf("ctgrsyst")->GetValue()==8 ){
      ;
    }else if( fl_2sveto &&
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  TFile outfile( Form("pdf_syst%d/makepdf_%s_lep%d_%s_%d.root", fl_multi,ctgry[fl_ctgry], fl_lep, fl_par, tag_cnt), "RECREATE" );
  can->Write();
