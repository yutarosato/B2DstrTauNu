...
const Char_t*  infile       = "~/dstrtaunu/ana/NB/hbk/hbk_mode0_test13_nor_setAA_HH/gMC_*s0[01]*.root";
const Double_t scale_factor = 2; // N -> N/scale
const Char_t*  branch       = "nb_mode0_test13_nor";
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  THStack* hs   = new THStack("NB", "NB;NB");
  ...
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  THStack* hs_norm    = new THStack("NB", "NB (Normalizaed);NB");
  THStack* hs_norm_sb = new THStack("NB", "NB (sig : bkg) (Normalizaed);NB");
  ...
  Double_t      summary1[10]; // 0(nbcut), 1(efficiency), 2(purity), 3(significance), 4(significanceE), 5(rejection ratio), 
  Double_t      summary2[10]; // 6(sig after cut), 7(bkg after cut), 8(sig before cut), 9(bkg before cut)
  make_eff_pur_sign( g1_eff, g1_sign, summary1, hist[0]          ); // efficiency   v.s. purity, significance v.s. nbout
  ...
  tex1->DrawTextNDC( 0.15,0.90, Form("NB > %.2f : eff = %.2f, purity = %.2f", summary1[0], summary1[1], summary1[2]) );
  tex1->DrawTextNDC( 0.15,0.86, Form("   significance = %.1f (S = %.0f, B = %.0f)", summary1[3], summary1[6], summary1[7] ) );
  TText* tex2 = new TText();
  tex2->SetTextColor(3);
  tex2->SetTextSize(0.04);
  tex2->DrawTextNDC( 0.15,0.82, Form("NB > %.2f : eff = %.2f, purity = %.2f", summary2[0], summary2[1], summary2[2]) );
  tex2->DrawTextNDC( 0.15,0.78, Form("   significance = %.1f (S = %.0f, B = %.0f)", summary2[3], summary2[6], summary2[7] ) );
  
  c1->cd(3);
  TMultiGraph* mg = new TMultiGraph( "significance", "significance v.s. NB; NB; significance" );
  mg->Add( g1_sign );
  mg->Add( g2_sign );
  mg->Draw("APL");
