...
//const Char_t*  selection_common_mc = "(ctgrsyst!=7&&ctgrsyst!=8)"; // veto D(*)(2S) // tmppppppppppppppp
...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  TTree* c_gmc_cut = c_gmc->CopyTree( Form("(%s)&&(%s)", sTmp.str().c_str(),selection_common_mc) );
  std::cout << "[Category] " << ctgry[fl_ctgry]                       << std::endl;
  std::cout << "[Cut] "      << sTmp.str().c_str()                    << std::endl
	    << "[Cut] "      << selection_common_mc                   << std::endl;
  ...
  TTree* c_npmc_cut = c_npmc->CopyTree( Form("(%s)&&(%s)", sTmp.str().c_str(),selection_common_mc) );
  std::cout << "[Category] " << ctgry[fl_ctgry]                        << std::endl;
  std::cout << "[Cut] "      << sTmp.str().c_str()                     << std::endl
	    << "[Cut] "      << selection_common_mc                    << std::endl;
  ...
  for( Int_t ihist=0; ihist<nhist; ihist++ ){
    ...
    hist1_eecl[ihist]->SetXTitle( "E'_{ECL} [GeV]" );
  }
  ...
  for( Int_t ievt=0; ievt<c_gmc_cut->GetEntries(); ievt++ ){
    ...
    if( c_gmc_cut->GetLeaf(nb_brname)->GetValue() > 0.0 ) hist1_eecl[2]->Fill( c_gmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    else                                                  hist1_eecl[1]->Fill( c_gmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist1_nb  [0]->Fill( c_gmc_cut->GetLeaf(  nb_brname)->GetValue(), weight );
    hist1_eecl[0]->Fill( c_gmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist2        ->Fill( c_gmc_cut->GetLeaf(  nb_brname)->GetValue(),
			 c_gmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
  }
  ...
  for( Int_t ievt=0; ievt<c_dststmc_cut->GetEntries(); ievt++ ){
    ...
    if( c_dststmc_cut->GetLeaf(nb_brname)->GetValue() > 0.0 ) hist1_eecl[2]->Fill( c_dststmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    else                                                      hist1_eecl[1]->Fill( c_dststmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist1_nb  [0]->Fill( c_dststmc_cut->GetLeaf(  nb_brname)->GetValue(), weight );
    hist1_eecl[0]->Fill( c_dststmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist2        ->Fill( c_dststmc_cut->GetLeaf(  nb_brname)->GetValue(),
			 c_dststmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
  }
  ...
  for( Int_t ievt=0; ievt<c_npmc_cut->GetEntries(); ievt++ ){
    ...
    if( c_npmc_cut->GetLeaf(nb_brname)->GetValue() > 0.0 ) hist1_eecl[2]->Fill( c_npmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    else                                                   hist1_eecl[1]->Fill( c_npmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist1_nb  [0]->Fill( c_npmc_cut->GetLeaf(  nb_brname)->GetValue(), weight );
    hist1_eecl[0]->Fill( c_npmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
    hist2        ->Fill( c_npmc_cut->GetLeaf(  nb_brname)->GetValue(),
			 c_npmc_cut->GetLeaf("eecl_exp" )->GetValue(), weight );
  }
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  can->Print( Form("pic/makepdf_%s_lep%d_%s.ps",ctgry[fl_ctgry],fl_lep, fl_par) );
  //can->Print( Form("pic/makepdf_%s_lep%d_%s.eps",ctgry[fl_ctgry],fl_lep, fl_par) ); // for belle note

  TFile outfile( Form("pic/makepdf_%s_lep%d_%s.root", ctgry[fl_ctgry], fl_lep, fl_par), "RECREATE" );
  can->Write();
